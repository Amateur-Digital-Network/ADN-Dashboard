...
install_update_server() {
  ...
  if [ ! -f adn.cfg ]; then
    ...
    sed -i 's|LOG_FILE: /dev/null|LOG_FILE: /opt/adnserver/log/adnserver.log|' adn.cfg
...
ExecStart=/usr/bin/python3 /opt/adnserver/bridge_master.py -c /opt/adnserver/config/adn.cfg
Restart=on-abort
