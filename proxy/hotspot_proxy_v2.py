...
class Proxy(DatagramProtocol):
    ...
    def __init__(self,Master,ListenPort,connTrack,peerTrack,blackList,IPBlackList,Timeout,Debug,ClientInfo,DestportStart,DestPortEnd,privHelper,rptlTrack,db_proxy):
        self.master = Master
            ...
            if _command == DMRD:                # DMRData -- encapsulated DMR data frame
                ...
                if _peer_id in self.peerTrack:
                    ...
                    if data[8:].upper().startswith(b'PASS='):
                        _psswd = data[13:]
                        if len(_psswd) >= 6:
                            dk = pbkdf2_hmac('sha256', _psswd, b'HamDMR', 2000).hex()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', action='store', dest='CONFIG_FILE', help='/full/path/to/config.file')
